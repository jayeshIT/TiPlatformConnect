(function() {
	...
	var linkedin = require('linkedin').Linkedin({
		consumerKey : 'rqqdcsikdga6',
		consumerSecret : 'VdokpSBTWJxtFq2G',
	...
	var tumblr = require('tumblr').Tumblr({
		consumerKey : 'q26pvyJ1qMxVLQfQT5aT5CdlOP4YjM8OFCx5lt54catQ9P0Q7I',
		consumerSecret : 'TyGIgyUS9upLxwiOR4WSrZiwWIfl3gaIiHMY0sFKN8YBffhLk',
	...
	var google = require('google').Google({
		consumerKey : '392532354152-92cb595he10caa7popsui66de17bq9h6.apps.googleusercontent.com',
		consumerSecret : 'miOX8IXD1pEQKKF-vb_fwDDn',
		...
		if (Ti.Geolocation.locationServicesEnabled) {
			Ti.Geolocation.getCurrentPosition(function(e) {
				if (e.error) {
					...
					foursquare.request('v2/venues/search', {
	...
	var googleAuthorize = function(event) {
		google.addEventListener('login', function(e) {
			if (e.success) {
				...
				google.request('https://www.google.com/m8/feeds/contacts/default/full', {
					alt : 'json'
				}, {}, 'GET', function(e) {
					if (e.success) {
						var json = JSON.parse(e.result.text);
	...
	var evernoteRow = Ti.UI.createTableViewRow({
		height : Ti.UI.FILL,
		touchEnabled : false,
		selectionStyle : Ti.UI.iPhone.TableViewCellSelectionStyle.NONE
	});
	...
	var evernoteLabel = Ti.UI.createLabel({
		left : 10,
		text : 'Sign in with Evernote'
	});
	evernoteRow.add(evernoteLabel);

	var evernoteSwitch = Ti.UI.createSwitch({
		right : 10,
	...
	var evernoteAuthorize = function(event) {
		evernote.addEventListener('login', function(e) {
			if (e.success) {
				Ti.App.Properties.setString("evernoteAccessTokenKey", e.accessTokenKey);
				evernoteLabel.setText("Evernote is authorized");
				evernoteSwitch.value = true;
				evernoteRow.add(evernoteSwitch);
				evernoteRow.touchEnabled = false;
				evernoteRow.selectionStyle = Ti.UI.iPhone.TableViewCellSelectionStyle.NONE;
				if (event) {
					evernoteRow.removeEventListener('click', evernoteAuthorize);
				}
			} else {
				alert("Authorization is failure");
			}
		});
